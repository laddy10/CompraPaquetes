...
public class Paq400MBWFTSalud4Beneficiarios implements Task {

    String precioTexto = PRECIO_$6000_3DIAS;
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                EsperarYClickSeleccionaEnUltimoMensaje.conTimeout(20),
                WaitFor.aTime(2000),
                ClickElementByText.clickElementByText(TODO_INCLUIDO_400MB_WFT_SALUD_4_BENEFICIARIOS));

        CapturaDePantallaMovil.tomarCapturaPantalla("captura_pantalla");

        actor.attemptsTo(
                ClickTextoQueContengaX.elTextoContiene(ENVIAR2),
                ValidarTextoQueContengaX.elTextoContiene(PRECIO_$6000_3DIAS),
                ValidarTextoQueContengaX.elTextoContiene(TODO_INCLUIDO_400MB_WFT_SALUD_4_BENEFICIARIOS));

        // Guardar información para validación en CMAX
        actor.remember("VALOR_COMPRA", valorFormateado);
        actor.remember("PAQUETE_CANAL", TODO_INCLUIDO_400MB_WFT_SALUD_4_BENEFICIARIOS);
    ...
    public static Performable paq400MBWFTSalud4Beneficiarios() {
        return instrumented(Paq400MBWFTSalud4Beneficiarios.class);
    }
}