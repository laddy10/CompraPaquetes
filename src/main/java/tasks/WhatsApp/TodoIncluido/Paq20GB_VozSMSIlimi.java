...
public class Paq20GB_VozSMSIlimi implements Task {

    String precioTexto = PRECIO_$23000_15DIAS;
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                EsperarYClickSeleccionaEnUltimoMensaje.conTimeout(20),
                WaitFor.aTime(2000),
                ClickElementByText.clickElementByText(TODO_INCLUIDO_20GB_VOZ_SMS_ILIM));

        CapturaDePantallaMovil.tomarCapturaPantalla("captura_pantalla");

        actor.attemptsTo(
                ClickTextoQueContengaX.elTextoContiene(ENVIAR2),
                ValidarTextoQueContengaX.elTextoContiene(PRECIO_$23000_15DIAS),
                ValidarTextoQueContengaX.elTextoContiene(TODO_INCLUIDO_20GB_VOZ_SMS_ILIM));

        // Guardar información para validación en CMAX
        actor.remember("VALOR_COMPRA", valorFormateado);
        actor.remember("PAQUETE_CANAL", TODO_INCLUIDO_20GB_VOZ_SMS_ILIM);
    ...
    public static Performable paq20GB_VozSMSIlimi() {
        return instrumented(Paq20GB_VozSMSIlimi.class);
    }
}