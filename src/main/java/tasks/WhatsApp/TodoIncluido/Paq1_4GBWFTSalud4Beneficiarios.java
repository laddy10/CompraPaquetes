...
public class Paq1_4GBWFTSalud4Beneficiarios implements Task {

    String precioTexto = PRECIO_$9000_6DIAS;
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                EsperarYClickSeleccionaEnUltimoMensaje.conTimeout(20),
                WaitFor.aTime(2000),
                ClickElementByText.clickElementByText(TODO_INCLUIDO_1_4GB_WFT_SALUD_4_BENEFICIARIOS));

        CapturaDePantallaMovil.tomarCapturaPantalla("captura_pantalla");

        actor.attemptsTo(
                ClickTextoQueContengaX.elTextoContiene(ENVIAR2),
                ValidarTextoQueContengaX.elTextoContiene(PRECIO_$9000_6DIAS),
                ValidarTextoQueContengaX.elTextoContiene(TODO_INCLUIDO_1_4GB_WFT_SALUD_4_BENEFICIARIOS));

        // Guardar información para validación en CMAX
        actor.remember("VALOR_COMPRA", valorFormateado);
        actor.remember("PAQUETE_CANAL", TODO_INCLUIDO_1_4GB_WFT_SALUD_4_BENEFICIARIOS);
    ...
    public static Performable paq1_4GBWFTSalud4Beneficiarios() {
        return instrumented(Paq1_4GBWFTSalud4Beneficiarios.class);
    }
}