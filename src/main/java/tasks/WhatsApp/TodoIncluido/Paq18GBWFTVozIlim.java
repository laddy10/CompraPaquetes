...
public class Paq18GBWFTVozIlim implements Task {

    String precioTexto = PRECIO_$43000_30DIAS;
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                EsperarYClickSeleccionaEnUltimoMensaje.conTimeout(20),
                WaitFor.aTime(2000),
                ClickElementByText.clickElementByText(TODO_INCLUIDO_18GB_WFT_VOZ));

        CapturaDePantallaMovil.tomarCapturaPantalla("captura_pantalla");

        actor.attemptsTo(
                ClickTextoQueContengaX.elTextoContiene(ENVIAR2),
                ValidarTextoQueContengaX.elTextoContiene(PRECIO_$43000_30DIAS),
                ValidarTextoQueContengaX.elTextoContiene(TODO_INCLUIDO_18GB_WFT_VOZ));

        // Guardar información para validación en CMAX
        actor.remember("VALOR_COMPRA", valorFormateado);
        actor.remember("PAQUETE_CANAL", TODO_INCLUIDO_18GB_WFT_VOZ);
    ...
    public static Performable paq18GBWFTVozIlim() {
        return instrumented(Paq18GBWFTVozIlim.class);
    }
}